...
// The vertical half, over Buffer C on iChannel2. Two one-dimensional passes compose
// into the two-dimensional Gaussian because that kernel is separable, and only
// because it is: a box or a tent would compose into something with corners.
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec2 stepUV = vec2(0.0, radius / float(FEATHER_TAPS) / iResolution.y);
	vec4 total = texture(iChannel2, uv);
	float weightTotal = 1.0;
	for (int i = 1; i <= FEATHER_TAPS; ++i)
	{
		float weight = featherTapWeight(i);
		vec2 offset = stepUV * float(i);
		total += texture(iChannel2, clamp(uv + offset, 0.0, 1.0)) * weight;
		total += texture(iChannel2, clamp(uv - offset, 0.0, 1.0)) * weight;
